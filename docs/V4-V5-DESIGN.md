...
- **Leases.** Grants are exact: pilld refuses any grant that would push the total over budget. Spending isn't: pilld counts tokens as they're reported and stops a session when its lease is spent, so a run can overrun by one step per running session. That overrun is documented, not hidden. To keep a step small, the enforcement hook caps the size of a Read and the number of Reads in parallel. Landed before v4 (`readcap.js`): one Read may bring in 32,000 bytes, and one message may make 4 Reads totalling 64,000 bytes. Measured on 2.1.282:
  - The largest single step fell from 112,553 tokens (five parallel 60KB Reads, no cap) to about 11,000 (`tools/it-readcap.mjs`). The same five files through Bash `cat` also came to about 11,000, because Claude Code limits what a Bash result brings in. A session's first message, which writes its system prompt to the cache, is about 10,000 on its own.
  - The 20,000-budget case from `tools/it-pause.mjs` (two workers and the orchestrator, no big reads) ended 11,247, 11,331 and 11,348 over, the same as before the cap: its overrun is three sessions' first steps.
  - A 20,000 budget with one worker reading five 60KB files in parallel ended 4,583, 4,285 and 5,485 over.
  - **Worst-case overrun, as measured: about 11,000 tokens per running session**, one step each. That is a measurement under these caps, not a guarantee: a step is whatever enters the context at once, and a tool the cap doesn't cover could make one larger.
