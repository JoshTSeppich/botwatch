...
| **Fullscreen on raise is off** unless `PILL_FULLSCREEN=1` | Kept off pending a decision. The original reason — the overlay can't draw over a fullscreen Space — **no longer holds**: measured 2026-09-24 on macOS 26.3.1, the pill stays on screen (window server: on-screen, layer 1000; screenshots before and after) over a native-fullscreen Terminal.app window, including when `AXFullScreen` is set true, which is the exact call the raise makes. Not measured: other terminals, older macOS |
