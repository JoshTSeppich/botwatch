...
| Fullscreen on raise | On, as the spec says; `PILL_FULLSCREEN=0` turns it off | It was opt-in while the pill disappeared over a fullscreen Space. On macOS 26.3.1 that no longer reproduces: the raise's own fullscreen call was measured on Terminal.app and iTerm2, and the pill stayed on top of both. Other terminals are unmeasured. |
